...
评估维度：
1. 功能范围数量（1个/2-3个/>3个）
2. 涉及文件估算（单文件/2-5文件/>5文件）
3. 技术复杂度（简单CRUD/中等业务逻辑/复杂架构）
4. 依赖关系（无依赖/少量依赖/复杂依赖）

评分规则：
- 简单任务（0-3分）：单一功能，单文件，无复杂依赖
- 中等任务（4-6分）：2-3个功能，2-5文件，少量依赖
- 复杂任务（7-10分）：多功能，>5文件，复杂依赖或架构级
```
...
简单任务 → 直接实现
中等任务 → writing-plans
复杂任务 → team（默认）或 autopilot/ralph（根据需求）
```

#### 步骤 3：输出推荐结果并调用选择框

**必须先输出需求总结和推荐理由，然后调用 AskUserQuestion 工具：**
...
**根据 AskUserQuestion 返回的选择执行对应操作：**

1. **如果选择"使用推荐工作流"或"直接实现"**：
   - 记录决策到 operations-log.md
   - 执行推荐的工作流

2. **如果选择其他工作流名称**（autopilot/ralph/team/writing-plans）：
   - 记录决策到 operations-log.md
   - 调用对应的工作流技能

3. **向后兼容**：如果用户直接回复文本而非通过选择框：
   - **"开始|执行|go|start|a"** → 执行推荐的工作流
   - **工作流名称** → 执行指定的工作流
